//
// Copyright © 2021 Surya Software Systems Pvt. Ltd.
//
...
public struct ResponseBody<Res: RPCResponse, Err: RPCError>: Decodable {
    ...
    public func parseResponse(
        ...
        case 200:
            ...
            guard let data = response.body else {
                throw RPCRuntimeError.invalidResponseError(message: "No response body found from an RPC: \(rpcName)")
            }
            let rpcResponseBody: ResponseBody<Self.Res, Self.Err>
            do {
                rpcResponseBody = try decoder.decode(ResponseBody<Res, Err>.self, from: data)
            } catch let e {
                throw RPCRuntimeError.invalidResponseError(error: e)
            }
